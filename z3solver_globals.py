...
def least_whites(s: Solver, colored: list[list[BoolRef]], n: int) -> None:
    min_white = n//2
    for i in range(n):
...
def most_blacks(s: Solver, colored: list[list[BoolRef]], n: int) -> None:
    max_black = (n//2)+1
    for i in range(n):
...
def close_isolation(s: Solver, colored: list[list[BoolRef]], grid: list[list[int]], n: int) -> None:
    for i in range(n):
        for j in range(n-2):
